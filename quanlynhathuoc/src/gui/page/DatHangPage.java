...
public class DatHangPage extends javax.swing.JPanel {
    ...
    private void tableLayout() {
        ...
        String[] header = new String[]{"STT", "Mã đặt hàng", "Thời gian", "Tên nhân viên", "Tên khách hàng", "Tổng hóa đơn", "Trạng thái", "Xác Nhận"};
    ...
    public void loadTable(List<DatHang> list) {
        ...
        for (DatHang e : listDH) {
            modal.addRow(new Object[]{
                String.valueOf(stt),
                e.getId(),
                Formatter.FormatTime(e.getThoiGian()),
                e.getNhanVien().getHoTen(),
                e.getKhachHang().getHoTen(),
                Formatter.FormatVND(e.getTongTien()),
                e.getTrangThai(),
                "Thanh Toán"  // Đây là cột "Xác Nhận" với button "Thanh Toán"
            });
            stt++;
        }
    }
    ...
    private void btnInfoActionPerformed(java.awt.event.ActionEvent evt) {
        try {
            ...
            DetailDatHangDialog dialog = new DetailDatHangDialog(null, true, listCTDH);
    ...
    private void handleThanhToan(int row) {
        ...
        if ("Chưa thanh toán".equals(trangThai)) {
            ...
            hoaDon.setId(idDH);
            hoaDon.setTongTien(datHang.getTongTien()); // Giả sử bạn có phương thức này trong `DatHang`
            ...
            // Cập nhật bảng
            table.setValueAt("Đã thanh toán", row, 6); // Cập nhật trạng thái thành "Đã thanh toán"
