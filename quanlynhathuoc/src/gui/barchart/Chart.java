...
public class Chart extends JPanel {
	...
	private void renderLegendSeries(Graphics2D g2, BlankPlotChart chart, SeriesSize size, int index, double x,
			int legendIndex) {
		ModelLegend legend = legends.get(legendIndex);
		g2.setColor(legend.getColor());
		double seriesValues = chart.getSeriesValuesOf(model.get(index).getValues()[legendIndex], size.getHeight())
				* animate;
		g2.fillRect((int) (size.getX() + x), (int) (size.getY() + size.getHeight() - seriesValues), seriesSize,
				(int) seriesValues);
	}
